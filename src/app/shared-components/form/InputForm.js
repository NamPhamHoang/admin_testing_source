...
const InputForm = ({values, setValues, onSubmit, onHandleShowModal, modalType}) => {
    const people = [
    ...
    const onHandleChangeSelect = (type) => (e) => {
        switch(type) {
            case "BADGE":  setValues({
                ...values,
                typeOfBadge: e.value
            }); break;
            case "PEOPLE":  setValues({
                ...values,
                teamMates: Array.isArray(e)?e.map(x=> x.value):[]
            }); break;
            case "PROMISE":  setValues({
                ...values,
    ...
    return (
        <Form>
             <Form.Group className="mb-3" controlId="formBasicPassword">
                ...
                <Select options={badge} onChange = {onHandleChangeSelect("BADGE")} placeholder="Select A Promise Card"/>
            </Form.Group>
            <Form.Group className="mb-3" controlId="formBasicCheckbox">
                <Form.Text className="text-muted">
                    Select or enter a custom Promise Card.
                </Form.Text>
                <Select className="mb-3" options={promiseCard} onChange = {onHandleChangeSelect("PROMISE")} placeholder="Select A Promise Card"/>
                <Form.Control type="text" placeholder="Or enter a custom promise card" />
            </Form.Group>

            <Form.Text className="text-muted">
                Select the person to whom you want to send the offer from the list below
            </Form.Text>
            <Select className="mb-3" options={people} isMulti onChange = {onHandleChangeSelect("PEOPLE")}/>
